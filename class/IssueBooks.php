...
class IssueBooks {	
	...
	public function listIssuedBook(){		
		...
		while ($issueBook = $result->fetch_assoc()) { 				
			...
			$rows[] = '<button type="button" name="update" id="'.$issueBook["issuebookid"].'" class="btn btn-warning btn-sm update"><span class="glyphicon glyphicon-edit" title="Edit">Modificar</span></button>';
			$rows[] = '<button type="button" name="delete" id="'.$issueBook["issuebookid"].'" class="btn btn-danger btn-sm delete" ><span class="glyphicon glyphicon-remove" title="Delete">Eliminar</span></button>';
			$records[] = $rows;
			$count++;
		}
