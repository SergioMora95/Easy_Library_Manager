...
class Rack {	
	...
	public function listRack(){		
		...
		while ($rack = $result->fetch_assoc()) { 				
			...
			$rows[] = $rack['status'];							
			$rows[] = '<button type="button" name="update" id="'.$rack["rackid"].'" class="btn btn-warning btn-xs update"><span class="glyphicon glyphicon-edit" title="Edit">Modificar</span></button>';
